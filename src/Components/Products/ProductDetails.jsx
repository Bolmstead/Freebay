...
import { getProduct, addBid } from "../../Api.js";
import Container from "@mui/material/Container";
import TextField from "@mui/material/TextField";
import LoadingText from "../Common/LoadingText.jsx";
...
function ProductDetails() {
  ...
  const { id } = useParams();
  const { currentUser, setUpdateAppBar } = useUserContext();
    ...
    async function fetchProduct() {
      try {
        ...
        if (productResult.bidId) {
          productResult.bidDisplay = parseFloat(productResult.bidPrice).toFixed(2);
        } else {
          productResult.bidDisplay = parseFloat(productResult.startingBid).toFixed(2);
        }
        ...
        setFormErrors("Failed to load product details");
      }
    }
    setInfoLoaded(false);
    fetchProduct();
  ...
  async function handleSubmit(evt) {
    ...
    try {
      ...
      setFormErrors("Failed to place bid. Please try again.");
    }
  }
    ...
    <Container>
      ...
      <Grid container spacing={4} justifyContent="center" alignItems="center">
        <Grid item xs={12} md={6}>
          <img className={classes.media} src={product.imageUrl} />
        </Grid>
        <Grid item xs={12} md={6}>
          <Card className={classes.root} variant="outlined">
            <CardContent className={classes.content} justifyContent="center">
