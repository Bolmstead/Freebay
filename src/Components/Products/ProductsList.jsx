...
const ProductsList = () => {
  ...
  const { products, setProducts, updateProductsList, setUpdateProductsList } = useUserContext();
  const numOfLoadingCards = [1, 2, 3, 4, 5, 6, 7, 8];

  const query = new URLSearchParams(useLocation().search);
  const searchQueryObject = Object.fromEntries(new URLSearchParams(query));

  // call API to grab products based on search results
  useEffect(() => {
    async function getProductsInCategory() {
      const nextPageSearchQueryObject = searchQueryObject;
      const prevPageSearchQueryObject = searchQueryObject;
      const res = await getProducts(searchQueryObject);
      const productsResult = res.products;
      const numOfProductsInAuction = res.numOfProductsInAuction;
      ...
      if (numOfProductsInAuction - page * 24 > 0) {
        ...
        const nextSearchQueryString = new URLSearchParams(nextPageSearchQueryObject).toString();
      ...
      if (page > 1) {
        ...
        const prevSearchQueryString = new URLSearchParams(prevPageSearchQueryObject).toString();
    ...
    if (Object.keys(searchQueryObject)[0] === "subCategory") {
      ...
      const fashionSubCategories = ["Mens", "Womens", "Girls", "Boys", "Baby"];
